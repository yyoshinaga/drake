...
void ConveyorController<T>::CalcPoseOutput(const systems::Context<T>& context ,
    ...
    // This code is to stop the conveyor from moving.
    // Reason is because I'm working on other things at the moment
    for(int i = 0; i < FLAGS_num_of_platforms+1; i++){
        output->SetAtIndex(i,0);
    }
    if(false){
        drake::log()->info(location);
    }
