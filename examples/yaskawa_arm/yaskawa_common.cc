...
Matrix6<T> ComputeLumpedGripperInertiaInEndEffectorFrame(
    ...
    int yaskawa_instance, const std::string& end_effector_link_name,
    int wsg_instance) {
  ...
  const RigidBody<T>* end_effector = world_tree.FindBody(
      end_effector_link_name, "iiwa7", yaskawa_instance);
  Isometry3<T> X_WEE =
...
void VerifyYaskawaTree(const RigidBodyTree<double>& tree) {
...
void SetPositionControlledYaskawaGains(Eigen::VectorXd* Kp,
                                    Eigen::VectorXd* Ki,
                                    Eigen::VectorXd* Kd) {
...
void ApplyJointVelocityLimits(const MatrixX<double>& keyframes,
  ...
  DRAKE_ASSERT(velocities.rows() == kYaskawaArmNumJoints);
  ...
  for (int i = 0; i < velocities.rows(); i++) {
    ...
    velocity_ratios(i) =
        max_plan_velocity / (yaskawa_max_joint_velocities[i] * 0.9);
  }
...
robotlocomotion::robot_plan_t EncodeKeyFrames(
  ...
  plan.robot_name = "yaskawa";  // Arbitrary, probably ignored
