...
int DoMain() {
      ...
      (!FLAGS_urdf.empty() ? FLAGS_urdf : FindResourceOrThrow(kModelPath));
  {
    ...
    auto frame = tree->findFrame("iiwa_frame_ee", -1);
...
//   }
  ...
  simulator.Initialize();
    ...
    VectorX<double> initPosition(7);
    initPosition << 0, 0, 0, 0, 0, 0, 0; 
...
//   std::unique_ptr<RigidBodyPlant<double>> plant = nullptr;

//   const char* kModelPath =  "drake/manipulation/models/iiwa_description/urdf/iiwa7.urdf";
//   const char* const WSG_Urdf = "drake/manipulation/models/wsg_50_description/sdf/schunk_wsg_50.sdf";

//   const std::string urdf_kuka =
//       (!FLAGS_urdf.empty() ? FLAGS_urdf : FindResourceOrThrow(kModelPath));

//   // const std::string urdf_schunk =
//   //     (!FLAGS_urdf.empty() ? FLAGS_urdf : FindResourceOrThrow(WSG_Urdf));
...
//         // parsers::urdf::AddModelInstanceFromUrdfFileToWorld(
//         //     urdf_kuka, multibody::joints::kFixed, tree.get());

//         tree_builder->StoreDrakeModel("iiwa", kModelPath);

//         tree_builder->StoreDrakeModel("wsg", WSG_Urdf);
...
//         //Sets the robot base height using vector3
//         int id = tree_builder->AddFixedModelInstance("iiwa", Eigen::Vector3d::Zero(3));
...
//             // Adds a plant.
//         plant = builder.AddPlant(std::move(tree_));
//     }

//     // ModelInstanceInfo<double> iiwa_instance, wsg_instance; //, box_instance;

//     // std::unique_ptr<systems::RigidBodyPlant<double>> model_ptr = plant;
//     //      //, &box_instance);
//     // model_ptr->set_name("plant");

//     // auto model =
//     //     builder.template AddSystem<IiwaAndWsgPlantWithStateEstimator<double>>(
//     //         std::move(model_ptr), iiwa_instance, wsg_instance, ModelInstanceInfo<double>()); //, box_instance);
//     // model->set_name("plant_with_state_estimator");

//       // parsers::urdf::AddModelInstanceFromUrdfFile(
//       // urdf_schunk, multibody::joints::kFixed, RigidBodyFrame<double>("iiwa_link_ee", tree.FindBody("iiwa_link_ee"), schunk_tree.get());
...
//     drake::log()->info(kIiwaArmNumJoints);
//         drake::log()->info(num_joints);


//     DRAKE_DEMAND(num_joints % (kIiwaArmNumJoints+2) == 0);
//     const int num_iiwa = num_joints/kIiwaArmNumJoints;
// drake::log()->info(num_iiwa);
