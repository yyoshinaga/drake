...
/// force.
///
class ConveyorBeltPdController : public systems::LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(ConveyorBeltPdController)
  ...
  const systems::OutputPort<double>& get_generalized_velocity_output_port() const {
    drake::log()->info("Calcgeneralizedvelocityoutput pd controller");

    return get_output_port(generalized_velocity_output_port_);
  }

 private:
  void CalcGeneralizedVelocityOutput(
      const systems::Context<double>& context,
      systems::BasicVector<double>* output_vector) const;

  systems::InputPortIndex desired_state_input_port_{};
  systems::InputPortIndex state_input_port_{};
  systems::OutputPortIndex generalized_velocity_output_port_{};
...
class ConveyorBeltVelocityController : public systems::Diagram<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(ConveyorBeltVelocityController)
  ...
  ConveyorBeltVelocityController(double time_step = 0.05);
  ...
  const systems::InputPort<double>& get_desired_velocity_input_port() const {
  ...
  const systems::OutputPort<double>& get_generalized_velocity_output_port() const {
    drake::log()->info("Calcgeneralizedvelocityoutput in the header");
