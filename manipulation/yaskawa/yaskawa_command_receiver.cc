...
std::unique_ptr<AbstractValue> MakeCommandMessage() {
  return std::make_unique<Value<lcmt_iiwa_command>>();    //TODO: Create new lcmtypes for yaskawa
}
}  // namespace

YaskawaCommandReceiver::YaskawaCommandReceiver(int num_joints)
  ...
  groomed_input_ = &DeclareCacheEntry(
      "groomed_input", &YaskawaCommandReceiver::CalcInput,
...
const systems::InputPort<double>& YaskawaCommandReceiver::get_input_port() const {
  return LeafSystem<double>::get_input_port(0);
}
using OutPort = systems::OutputPort<double>;
const OutPort& YaskawaCommandReceiver::get_commanded_position_output_port() const {
  return LeafSystem<double>::get_output_port(0);
}
const OutPort& YaskawaCommandReceiver::get_commanded_torque_output_port() const {
  return LeafSystem<double>::get_output_port(1);
}

void YaskawaCommandReceiver::set_initial_position(
...
void YaskawaCommandReceiver::CalcInput(
  const Context<double>& context, lcmt_iiwa_command* result) const {
  if (!get_input_port().HasValue(context)) {
    throw std::logic_error("YaskawaCommandReceiver has no input connected");
  }
  ...
  *result = get_input_port().Eval<lcmt_iiwa_command>(context);
  ...
  if (result->num_joints != num_joints_) {
    throw std::runtime_error(fmt::format(
        "YaskawaCommandReceiver expected num_joints = {}, but received {}",
  ...
  if (result->num_torques == 0) {
    ...
    throw std::runtime_error(fmt::format(
        "YaskawaCommandReceiver expected num_torques = {}, but received {}",
        num_joints_, result->num_torques));
  }
}

YaskawaCommandReceiver::MapVectorXd YaskawaCommandReceiver::input_position(
...
YaskawaCommandReceiver::MapVectorXd YaskawaCommandReceiver::input_torque(
    const Context<double>& context) const {
  const auto& message = groomed_input_->Eval<lcmt_iiwa_command>(context);
  return Eigen::Map<const VectorXd>(
